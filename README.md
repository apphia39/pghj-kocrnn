...
- 학습 데이터셋: [AI Hub 한국어 글자체 이미지](https://aihub.or.kr/aidata/133), 직접 제작
- 테스트 데이터: [AI Hub 한국어 글자체 이미지](https://aihub.or.kr/aidata/133), 직접 제작

## Dataset
AI Hub 한국어 글자체 이미지에서는 **인쇄체**와 **필기체**를 이용하였고, 추가로 직접 데이터셋을 제작해 학습하였다.

### 1. Preprocessing
####  AI Hub 한국어 글자체 이미지 데이터셋 가공(htr)
I. AI Hub에서 필기체 데이터셋을 [다운로드](https://aihub.or.kr/aidata/133/download) 받는다.
II. 이미지 파일들은 kor_dataset/aihub_data/htr/images/ 폴더에 저장하고, 라벨링 파일은 kor_dataset/aihub_data/htr/ 폴더에 저장한다.
III. aihub_dataset.py를 열어 아래와 같이 변수 값을 수정한다.
```python3
data_type = 'htr'
labeling_filename = 'handwriting_data_info1.json'
```
IV. AI Hub 데이터셋의 라벨링 파일 구조는 아래와 같다.
```
...
```
V. 아래 파일을 실행시켜 데이터 전처리를 수행한다.
```
python3 aihub_dataset.py
```
이후 deep-text-recognition-benchmark/htr_data/ 폴더에 gt_test.txt, gt_train.txt, gt_validation.txt가 생성된다.

#### 직접 제작한 데이터셋 가공: finetuning_dataset.py
I. 해당 데이터셋을 다운로드 받는다.
II. 이미지 파일들은 kor_dataset/finetuning_data/made1/images/ 폴더에 저장하고, 라벨링 파일은 kor_dataset/aihub_data/made1/ 폴더에 저장한다.
III. finetuning_dataset.py를 열어 아래와 같이 변수 값을 수정한다.
```python3
data_type = 'made1'
```
IV. 라벨링 파일 구조는 아래와 같다.
```
# labels.txt
# {filename}.jpg {label}
0.jpg 여기가
...
python3 aihub_dataset.py
```
이후 deep-text-recognition-benchmark/made1_data/ 폴더에 gt_test.txt, gt_train.txt, gt_validation.txt가 생성된다.

### 2. lmdb data 만들기
I. Preprocessing이 완료되면, deep-text-recognition-benchmark/{xxx}_data 폴더에는 gt_{xxx}.txt 파일들이 존재한다.
II. get_images.py 파일을 deep-text-recognition-benchmark/{xxx}_data 폴더로 복사한 뒤 test, train, validation 폴더를 생성한다.
```
...
```
III. kor_dataset에 있는 이미지 파일들을 현재 폴더로 가져와서 test, train, validation 폴더로 나눠준다.
```
python3 get_images.py
```
IV. 학습을 위해 lmdb data를 생성한다.
```
cd ../../
...
> **주의사항**
> 현재, 학습을 진행하게 되면 항상 saved_models/TPS-ResNet-BiLSTM-CTC-Seed1234 폴더 아래로 저장된다.
> 따라서 새로운 학습을 진행할 때마다 모델이 overwrite되는 것을 방지하기 위해서는 
> 1) --manualSeed 옵션에 매번 다른 값을 넣어주거나
> 2) mv명령어를 이용해 학습이 한 번 끝날 때마다 TPS-ResNet-BiLSTM-CTC-Seed1234 폴더명을 다른 이름으로 바꿔주도록 한다.

<br>

## Test
### 1. gt_test.txt 파일을 이용해 test할 경우
I. deep-text-recognition-benchmark/htr_data/test 폴더에 있는 이미지들을 테스트한다. 라벨링 파일은 gt_test.txt를 이용한다.
```
# gt_test.txt
...
...
I. test/images 폴더 아래 테스트할 이미지들을 넣고, test 폴더 아래 해당 테스트 이미지들에 대한 라벨링 파일(labels.txt)을 넣는다.
```
# labels.txt
...
10.jpg 여기에
...
왼쪽부터 순서대로 | Image Name | Real Text | Predicted Text | Confidence Score | Character Error Rate | 이다.
